...
class PublishResourceTestCase(ApiTestCase):
    """Base class for the `publish` resource tests."""
    ...
    def _get_target_class(self):
        ...
        """
        try:
            from content_api.publish import PublishResource
        except ImportError:
            self.fail("Could not import class under test (PublishResource).")
        else:
            return PublishResource


class ResourceConfigTestCase(PublishResourceTestCase):
    """Tests for the configuration of the `publish` resource."""
